...
BoolVector::BoolVector(int length) 
	: m_length(length) {
	assert(length >= 0);
	m_cellNumber = m_length / CellSize;
	m_cells = new Cell[m_cellNumber];
...
{
	m_cellNumber = other.m_cellNumber;
...
void BoolVector::swap(BoolVector& other)
{
	...
	std::swap(m_cellNumber, other.m_cellNumber);
...
void BoolVector::setAllBits(bool value) {
	for (size_t i = 0; i < m_cellNumber; ++i) {
		if (value) {
			m_cells[i] = ~0;
		}
		else {
...
int BoolVector::_excessRankNumber() const
{
	return (m_cellNumber * CellSize - m_length);
}
